import React from "react";

const About = () => {
  return (
    <section
  id="about"
  className="w-full py-20 bg-[#1e1e1e] text-gray-200 relative"
>
  <div className="max-w-5xl mx-auto px-8">
    <h2 className="text-3xl sm:text-4xl font-bold mb-6 text-center">About Me</h2>
    <p className="text-lg sm:text-xl leading-relaxed">
      I am Doulat Singh jadaun, a passionate software developer with expertise in full-stack development, DevOps, and embedded systems. I enjoy building innovative projects and continuously learning new technologies.
    </p>
    <p className="text-lg sm:text-xl leading-relaxed mt-4">
      Currently pursuing my Bachelor's in Electronics and Communication Engineering, I bring a unique perspective to problem-solving and software development.
    </p>
  </div>
</section>

  );
};

export default About;
